...
.. admonition:: Exercise
	...
	Can you design a command or set of commands that can do this from what we've already covered? Bonus points if you can do it in a loop, but it's not necessary!
...
.. admonition:: Solution
	...
	.. code-block:: bash
		...
		$ vsearch --fastx_filter input.fasta --fastq_minlen 418 --fastq_maxlen 418 --fastaout output418.fasta
		$ vsearch --fastx_filter input.fasta --fastq_minlen 421 --fastq_maxlen 421 --fastaout output421.fasta
	...
	.. code-block:: bash 
		...
		> do \
		> 	vsearch --fastx_filter input.fasta --fastq_minlen $l \
		> 	--fastq_maxlen $l --fastaout output_${l}_ctrim.fasta; \ 
		> done &&\
		> cat *ctrim.fasta > output.fasta
